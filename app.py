...
selected_menu = st.sidebar.radio("이동할 페이지를 선택하세요:", ["🏠 메인 홈"])

st.sidebar.markdown("---")
if st.session_state.logged_in:
    st.sidebar.success(f"🔒 {st.session_state.current_user} 계정 인증 완료")
...
# ==============================================================================
# 🔥 [핵심 요청 반영] 메인 홈 가장 밑에 들어가는 부장 전용 비밀 관리 공간
# ==============================================================================
st.markdown('<div class="section-title">🔒 부장 전용 시스템 시크릿 포털</div>', unsafe_allow_html=True)
st.write("이 공간은 오직 동아리 부장만 패스워드를 입력하여 제어할 수 있는 메인 홈 하단 관리창입니다.")

# 부장 확인용 패스워드 입력창
boss_password = st.text_input("부장 전용 마스터 패스워드를 입력하세요:", type="password", placeholder="비밀번호 입력 시 하단에 관리 도구가 나타납니다.")

# 부장 비밀번호인 '2025'가 완벽히 입력되었을 때만 밑에 숨겨진 페이지가 등장!
if boss_password == "2025":
    st.markdown('<div class="admin-container">', unsafe_allow_html=True)
    st.subheader("👑 DUIT 부장 마스터 관리 콘솔 열림")
    st.info("비밀번호 인증 성공! 메인 홈 하단에서 실시간으로 데이터를 관리할 수 있습니다.")
    st.markdown("---")
    
    # [파트 1: 이전 활동 보고 데이터(부장목록) 유동적 제어]
    st.markdown("### 🚀 '이전 활동 보고' 부장목록 실시간 수정")
    with st.form("boss_log_add_form"):
        st.write("상단 동아리 게시판의 [이전 활동 보고]에 신규 데이터를 추가합니다.")
        input_log = st.text_input("추가할 내역 입력 (예: '07월: 교내 알고리즘 경진대회 참가'):")
        add_log_btn = st.form_submit_button("➕ 메인 화면 게시판에 즉시 등록")
        
        if add_log_btn and input_log.strip():
            st.session_state.boss_log_list.append(input_log.strip())
            st.success("게시판 목록에 성공적으로 저장되었습니다!")
            st.rerun()
    ...
    if len(st.session_state.tastes_list) == 0:
        ...
        for t_item in st.session_state.tastes_list:
            with st.expander(f"⚙️ [{t_item['category']}] 카드 ID: {t_item['id']} 내용 제어"):
                edit_input = st.text_input("글 내용 강제 수정", value=t_item['text'], key=f"master_edit_{t_item['id']}")
    ...
    if boss_password.strip() != "":
        st.error("부장 전용 비밀번호가 일치하지 않습니다. 다시 입력해주세요.")
